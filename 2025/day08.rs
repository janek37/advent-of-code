...
pub fn main() {
    ...
    let mut union_sets = UnionSets::new(&junctions);
    println!("{}", make_connections(&pairs, &mut union_sets));
}

fn get_circuit_sizes(junctions: &[Point], pairs: &[(Point, Point)]) -> Vec<usize> {
    let mut union_sets = UnionSets::new(&junctions);
    make_connections(pairs, &mut union_sets);
    let mut circuit_sizes: Vec<usize> = junctions
        .iter()
        .filter(|&&j| union_sets.is_root(j))
        .map(|&j| union_sets.get_count(j)).collect();
...
fn make_connections(pairs: &[(Point, Point)], union_sets: &mut UnionSets<Point>) -> i32 {
    let mut last_connection = pairs[0];
    let mut connection_count = 0;
    let max_connections = union_sets.counts.len() - 1;
    for &(j1, j2) in pairs {
        if !union_sets.are_connected(j1, j2) {
            union_sets.union(j1, j2);
